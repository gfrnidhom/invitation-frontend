...
export default function BlushRomantic({ payload }) {
            ...
            <div className={`transition-opacity duration-1000 ${isOpen?'opacity-100':'opacity-0 pointer-events-none'}`}>
                <div className="flex flex-col lg:flex-row min-h-screen">
                    <div className="sl-br w-full lg:w-[55%] bg-[#8b3a4a] relative flex flex-col justify-end p-8 md:p-12 lg:p-16">
                        ...
                        {invitation?.music_url&&<button onClick={ta} className="absolute bottom-8 right-8 lg:bottom-1/2 lg:-right-6 lg:translate-y-1/2 z-50 w-12 h-12 rounded-full bg-[#8b3a4a] border border-[#b76e79]/30 flex items-center justify-center hover:border-[#b76e79]/60 transition-all shadow-2xl">{isPlaying?<svg className="w-5 h-5 ms-br rg-txt" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2z"/></svg>:<svg className="w-5 h-5 rg-txt" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}</button>}
                    </div>
