...
export default function MonoChromeII({ payload, audioController }) {
    ...
    return (
        <div className={`min-h-screen bg-[var(--theme-bg,#2c2420)] text-[var(--theme-text,#e8ddd0)] ${sourceSans.className} overflow-hidden`}>
            ...
            <div className={`co2 ${isOpen?'open':''}`}>
                <div className="relative w-full h-full flex flex-col items-center justify-center bg-[#1f1915]">
                    ...
                    <div className="relative z-10 text-center px-6 flex flex-col items-center">
                        ...
                        <button onClick={ho} className={`${oswald.className} border border-[var(--theme-accent,#d4c5b3)]/30 px-10 py-4 text-[10px] tracking-[.4em] uppercase text-[var(--theme-accent,#d4c5b3)]/70 hover:bg-[var(--theme-accent,#d4c5b3)] hover:text-[#2c2420] transition-all duration-500`}>Buka Undangan</button>
            ...
            <div className={`transition-opacity duration-1000 ${isOpen?'opacity-100':'opacity-0 pointer-events-none'}`}>
                <div className="flex flex-col lg:flex-row min-h-screen">
                    {/* LEFT */}
                    <div className="sl2 w-full lg:w-[70%] bg-[var(--theme-bg,#2c2420)] relative flex flex-col justify-end p-8 md:p-12 lg:p-16">
                        ...
                        {invitation?.music_url&&<MusicPlayer audioController={audioController} btnBg="bg-[var(--theme-bg,#2c2420)]" btnColor="sep" btnBorder="border-[var(--theme-accent,#d4c5b3)]/15 shadow-2xl" />}
                    </div>
                    ...
                    <div ref={rpRef} className="w-full lg:w-[30%] lg:h-screen lg:overflow-y-auto sh bg-[var(--theme-bg,#2c2420)]">
                        ...
                        <section className="px-8 pb-20">
                            <div className="text-center mb-12 m2-rv"><h2 className={`${oswald.className} text-2xl tracking-[.2em] uppercase text-[var(--theme-text,#e8ddd0)]`}>Bride & Groom</h2><p className="text-[10px] text-[var(--theme-text,#e8ddd0)]/15 tracking-[.3em] uppercase mt-2">Assalamualaikum Wr. Wb.</p><p className="text-[10px] text-[var(--theme-text,#e8ddd0)]/25 mt-3 max-w-md mx-auto leading-relaxed font-light">Tanpa mengurangi rasa hormat, kami bermaksud mengundang Bapak/Ibu/Saudara/i untuk menghadiri acara pernikahan kami:</p></div>
                            {/* Bride */}
                            <div className="text-center mb-6 m2-rv" data-delay="1">
                                {brP&&<div className="hex-frame w-44 h-52 mx-auto mb-6 bg-[var(--theme-accent,#d4c5b3)]/10"><img src={brP} alt="Bride" className="w-full h-full object-cover sepia hover:sepia-0 transition-all duration-700"/></div>}
                                <p className={`${playfair.className} italic text-3xl sep mb-1`}>{invitation?.bride_name?.split(' ')[0]}</p>
                                <h3 className={`${oswald.className} text-xl tracking-[.1em] uppercase text-[var(--theme-text,#e8ddd0)] mb-3`}>{invitation?.bride_full_name||invitation?.bride_name}</h3>
                                <p className="text-sm text-[var(--theme-text,#e8ddd0)]/30 font-light">Putri {invitation?.bride_child_order ? `${invitation.bride_child_order} ` : ""}dari</p>
                                <p className="text-sm text-[var(--theme-text,#e8ddd0)]/50">{invitation?.bride_father||'Bapak'} & {invitation?.bride_mother||'Ibu'}</p>
                                {invitation?.bride_instagram && (
                                    <a href={`https://instagram.com/${invitation.bride_instagram}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-[var(--theme-text,#e8ddd0)]/60 hover:text-[var(--theme-text,#e8ddd0)] transition-colors font-medium">
                            ...
                            <div className="text-center m2-rv" data-delay="2">
                                ...
                                <h3 className={`${oswald.className} text-xl tracking-[.1em] uppercase text-[var(--theme-text,#e8ddd0)] mb-3`}>{invitation?.groom_full_name||invitation?.groom_name}</h3>
                                <p className="text-sm text-[var(--theme-text,#e8ddd0)]/30 font-light">Putra {invitation?.groom_child_order ? `${invitation.groom_child_order} ` : ""}dari</p>
                                <p className="text-sm text-[var(--theme-text,#e8ddd0)]/50">{invitation?.groom_father||'Bapak'} & {invitation?.groom_mother||'Ibu'}</p>
                                {invitation?.groom_instagram && (
                                    <a href={`https://instagram.com/${invitation.groom_instagram}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-[var(--theme-text,#e8ddd0)]/60 hover:text-[var(--theme-text,#e8ddd0)] transition-colors font-medium">
                        ...
                        {/* Events */}
                        <section className="px-8 pb-20">
                            <div className="text-center mb-12 m2-rv"><h2 className={`${oswald.className} text-2xl tracking-[.2em] uppercase text-[var(--theme-text,#e8ddd0)]`}>Wedding</h2><p className={`${playfair.className} italic text-4xl sep -mt-1`}>Event</p></div>
                        ...
                        <Gallery 
                            ...
                            accentText="text-[var(--theme-text,#e8ddd0)]"
                            subtitleText="sep"
                            borderColor="border-[var(--theme-accent,#d4c5b3)]/15"
                        />

                        {/* QR Checkin */}
                        <div className="px-8"><QrCheckin guest={guest} sectionBg="bg-transparent" titleFont={oswald.className} textColor="text-[var(--theme-text,#e8ddd0)]" borderStyle="border-[var(--theme-accent,#d4c5b3)]/15"/></div>

                        {/* Wishes */}
                        <section className="px-8 pb-20 m2-rv">
                            <div className="text-center mb-12"><h2 className={`${oswald.className} text-2xl tracking-[.2em] uppercase text-[var(--theme-text,#e8ddd0)]`}>Wedding</h2><p className={`${playfair.className} italic text-4xl sep -mt-1`}>Wishes</p></div>
                            <div className="m2cw rounded-3xl p-8">
                                <form onSubmit={sw} className="space-y-4">
                                    <div><label className={`${oswald.className} block text-[9px] tracking-[.2em] uppercase text-[#2c2420]/30 mb-2`}>Nama</label><input type="text" value={ni} onChange={e=>setNi(e.target.value)} className="w-full bg-[var(--theme-bg,#2c2420)]/5 border border-[#2c2420]/10 rounded-xl px-5 py-3.5 text-sm text-[#2c2420] focus:outline-none focus:border-[var(--theme-accent,#d4c5b3)]/50 transition-colors" placeholder="Nama Anda..."/></div>
                                    <div><label className={`${oswald.className} block text-[9px] tracking-[.2em] uppercase text-[#2c2420]/30 mb-2`}>Ucapan</label><textarea value={mi} onChange={e=>setMi(e.target.value)} className="w-full bg-[var(--theme-bg,#2c2420)]/5 border border-[#2c2420]/10 rounded-xl px-5 py-3.5 text-sm text-[#2c2420] h-28 resize-none focus:outline-none focus:border-[var(--theme-accent,#d4c5b3)]/50 transition-colors" placeholder="Tulis ucapan..."/></div>
                                    <button type="submit" disabled={sub} className={`${oswald.className} w-full bg-[var(--theme-bg,#2c2420)] text-[#f5f0ea] py-4 rounded-xl text-[10px] tracking-[.3em] uppercase hover:bg-[#1f1915] transition-colors disabled:opacity-50`}>{sub?'Mengirim...':'Kirim Ucapan'}</button>
                                </form>
                                {ws.length>0&&<div className="mt-8 space-y-3 max-h-[300px] overflow-y-auto sh">{ws.map((m,i)=>(<div key={i} className="bg-[var(--theme-bg,#2c2420)]/5 rounded-2xl p-4 border border-[#2c2420]/6"><p className="text-sm text-[#2c2420]/60 font-light">{m.message}</p><div className="flex items-center gap-2 mt-2"><div className="w-6 h-6 rounded-full bg-[var(--theme-bg,#2c2420)] flex items-center justify-center"><span className="text-[#f5f0ea] text-[10px] font-bold">{m.name?.charAt(0)?.toUpperCase()}</span></div><p className="text-xs text-[#2c2420]/30">{m.name}</p></div></div>))}</div>}
                            </div>
                        </section>

                        {/* Gift */}
                        {invitation?.gift_accounts&&invitation.gift_accounts.length>0&&<section className="px-8 pb-20 m2-rv"><div className="text-center mb-12"><h2 className={`${oswald.className} text-2xl tracking-[.2em] uppercase text-[var(--theme-text,#e8ddd0)]`}>Wedding</h2><p className={`${playfair.className} italic text-4xl sep -mt-1`}>Gift</p></div><div className="space-y-4">{invitation.gift_accounts.map((acc, i) => (
