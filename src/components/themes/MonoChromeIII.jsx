...
export default function MonoChromeIII({ payload, audioController }) {
    ...
    return (
        <div className={`min-h-screen bg-white text-[var(--theme-text,#1a1a1a)] ${dmSans.className} overflow-hidden`}>
            ...
            <div className={`co3 ${isOpen?'open':''}`}>
                <div className="relative w-full h-full flex flex-col items-center justify-center bg-white">
                    ...
                    <div className="relative z-10 text-center px-6 flex flex-col items-center">
                        <div className="w-20 h-px bg-[var(--theme-bg,#1a1a1a)]/15 mb-8"/>
                        <p className={`${dmSans.className} text-[10px] tracking-[.6em] uppercase text-[var(--theme-text,#1a1a1a)]/35 mb-6 font-medium`}>The Wedding Of</p>
                        <h1 className={`${dmSerif.className} text-6xl md:text-8xl lg:text-9xl text-[var(--theme-text,#1a1a1a)] mb-4`}>{invitation?.groom_name?.split(' ')[0]}</h1>
                        <p className={`${lora.className} italic text-4xl text-[var(--theme-text,#1a1a1a)]/20 my-2`}>&</p>
                        <h1 className={`${dmSerif.className} text-6xl md:text-8xl lg:text-9xl text-[var(--theme-text,#1a1a1a)] mb-6`}>{invitation?.bride_name?.split(' ')[0]}</h1>
                        <p className="text-[10px] text-[var(--theme-text,#1a1a1a)]/25 tracking-[.4em] uppercase mb-10">{ed.toLocaleDateString('id-ID',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</p>
                        {guestName&&<div className="mb-8"><p className="text-[9px] text-[var(--theme-text,#1a1a1a)]/20 uppercase tracking-[.3em] mb-1">Kepada Yth.</p><p className={`${lora.className} italic text-2xl text-[var(--theme-text,#1a1a1a)]/60`}>{guestName}</p></div>}
                        <button onClick={ho} className={`${dmSans.className} bg-[var(--theme-bg,#1a1a1a)] text-[var(--theme-button-text,#ffffff)] px-10 py-4 text-[10px] tracking-[.4em] uppercase font-medium hover:bg-[var(--theme-accent,#333)] transition-all duration-500`}>Buka Undangan</button>
                        <div className="w-20 h-px bg-[var(--theme-bg,#1a1a1a)]/15 mt-8"/>
                    </div>
                </div>
            </div>
            ...
            <div className={`transition-opacity duration-1000 ${isOpen?'opacity-100':'opacity-0 pointer-events-none'}`}>
                <div className="flex flex-col lg:flex-row min-h-screen">
                    {/* LEFT - Dark panel */}
                    <div className="sl3 w-full lg:w-[70%] bg-[var(--theme-bg,#1a1a1a)] relative flex flex-col justify-end p-8 md:p-12 lg:p-16">
                        ...
                        {invitation?.music_url&&<MusicPlayer audioController={audioController} btnBg="bg-white" btnColor="text-[var(--theme-text,#1a1a1a)]" btnBorder="border-[#1a1a1a]/10 shadow-2xl" />}
                    </div>
                    ...
                    <div ref={rpRef} className="w-full lg:w-[30%] lg:h-screen lg:overflow-y-auto sh bg-white">
                        ...
                        <section className="py-20 px-8 text-center m3-rv">
                            <p className={`${dmSans.className} text-[10px] tracking-[.5em] uppercase text-[var(--theme-text,#1a1a1a)]/25 mb-4 font-medium`}>Save The Date</p>
                            <h2 className={`${dmSerif.className} text-3xl text-[var(--theme-text,#1a1a1a)] mb-2`}>{invitation?.groom_name?.split(' ')[0]} & {invitation?.bride_name?.split(' ')[0]}</h2>
                            <p className="text-[10px] text-[var(--theme-text,#1a1a1a)]/20 tracking-[.3em] uppercase mb-10">{ed.toLocaleDateString('id-ID',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</p>
                        ...
                        <section className="px-8 pb-20 m3-rv">
                            <div className="m3cl rounded-3xl p-8 text-center">
                                <p className={`${lora.className} italic text-sm leading-relaxed text-[var(--theme-text,#1a1a1a)]/50 mb-4`}>{invitation?.quotes||'"Dan di antara tanda-tanda kekuasaan-Nya diciptakan-Nya untukmu pasangan hidup dari jenismu sendiri supaya kamu dapat ketenangan hati dan dijadikannya kasih sayang di antara kamu."'}</p>
                                <p className={`${dmSans.className} text-[10px] text-[var(--theme-text,#1a1a1a)]/25 tracking-[.3em] uppercase font-medium`}>{invitation?.quotes_name || 'QS. Ar-Rum Ayat 21'}</p>
                            </div>
                        </section>

                        {/* Bride & Groom — Side-by-side on dark cards */}
                        <section className="px-8 pb-20">
                            <div className="text-center mb-12 m3-rv"><h2 className={`${dmSerif.className} text-3xl text-[var(--theme-text,#1a1a1a)]`}>Bride & Groom</h2><p className="text-[10px] text-[var(--theme-text,#1a1a1a)]/15 tracking-[.3em] uppercase mt-2">Assalamualaikum Wr. Wb.</p><p className="text-[10px] text-[var(--theme-text,#1a1a1a)]/30 mt-3 max-w-md mx-auto leading-relaxed font-light">Tanpa mengurangi rasa hormat, kami bermaksud mengundang Bapak/Ibu/Saudara/i untuk menghadiri acara pernikahan kami:</p></div>
                            ...
                            <div className="text-center my-4 m3-rv"><span className={`${lora.className} italic text-5xl text-[var(--theme-text,#1a1a1a)]/10`}>&</span></div>
                        ...
                        {/* Events */}
                        <section className="px-8 pb-20">
                            <div className="text-center mb-12 m3-rv"><h2 className={`${dmSerif.className} text-3xl text-[var(--theme-text,#1a1a1a)]`}>Wedding Event</h2></div>
                            {invitation?.events&&invitation.events.length>0?[...invitation.events].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)).map((ev,idx)=>(
                                <div key={idx} className="m3cl rounded-3xl p-8 mb-6 text-center m3-rv" data-delay={`${idx+1}`}>
                                    <h3 className={`${dmSerif.className} text-xl text-[var(--theme-text,#1a1a1a)] mb-4`}>{ev.name}</h3>
                                    <p className="text-sm text-[var(--theme-text,#1a1a1a)]/40 font-light mb-1">{ev.date?new Date(ev.date).toLocaleDateString('id-ID',{weekday:'long',day:'numeric',month:'long',year:'numeric'}):''}</p>
                                    <p className="text-sm text-[var(--theme-text,#1a1a1a)]/40 font-light mb-3">Pukul : {ev.time_start?.substring(0,5)||'TBA'} {ev.time_end?`- ${ev.time_end.substring(0,5)}`:'- Selesai'} WIB</p>
                                    {ev.location&&<p className="text-sm text-[var(--theme-text,#1a1a1a)]/55 pt-3 border-t border-[#1a1a1a]/5">{ev.location}</p>}
                                    {getMapUrl(ev)&&<MapLocationButton item={ev} className={`${dmSans.className} inline-flex items-center gap-2 bg-[var(--theme-bg,#1a1a1a)] text-[var(--theme-button-text,#ffffff)] px-6 py-3 text-[10px] tracking-[.2em] uppercase font-medium hover:bg-[var(--theme-accent,#333)] transition-all duration-500 mt-4`} buttonText="Lihat Lokasi"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"/></svg><span>Lihat Lokasi</span></MapLocationButton>}
                                </div>
                            )):<div className="m3cl rounded-3xl p-10 text-center"><h3 className={`${dmSerif.className} text-xl text-[var(--theme-text,#1a1a1a)] mb-2`}>Acara Pernikahan</h3></div>}
                        </section>

                        {invitation?.love_stories&&invitation.love_stories.length>0&&<section className="px-8 pb-20"><div className="text-center mb-12 m3-rv"><h2 className={`${dmSerif.className} text-3xl text-[var(--theme-text,#1a1a1a)]`}>Our Love Story</h2></div>{[...invitation.love_stories].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)).map((s,i)=>(<div key={s.id||i} className="m3cl rounded-3xl p-8 text-center mb-6 m3-rv" data-delay={`${i+1}`}><h3 className={`${dmSerif.className} text-lg text-[var(--theme-text,#1a1a1a)] mb-4`}>{s.title}</h3><p className="text-sm text-[var(--theme-text,#1a1a1a)]/50 leading-relaxed font-light">{s.description}</p>{s.photo&&<div className="mt-6 rounded-xl overflow-hidden"><img src={gp(s.photo)} alt={s.title} className="w-full h-44 object-cover grayscale"/></div>}</div>))}</section>}
                        ...
                        <Gallery 
                            ...
                            titleSize="text-3xl"
                            accentText="text-[var(--theme-text,#1a1a1a)]"
                            subtitleText="text-[var(--theme-text,#1a1a1a)]/50"
                            borderColor="border-[#1a1a1a]/10"
                        />

                        <div className="px-8"><QrCheckin guest={guest} sectionBg="bg-transparent" titleFont={dmSerif.className} textColor="text-[var(--theme-text,#1a1a1a)]" borderStyle="border-[#1a1a1a]/8"/></div>

                        {/* Wishes */}
                        <section className="px-8 pb-20 m3-rv">
                            <div className="text-center mb-12"><h2 className={`${dmSerif.className} text-3xl text-[var(--theme-text,#1a1a1a)]`}>Wedding Wishes</h2></div>
                            <div className="m3cl rounded-3xl p-8">
                                <form onSubmit={sw} className="space-y-4">
                                    <div><label className={`${dmSans.className} block text-[9px] tracking-[.2em] uppercase text-[var(--theme-text,#1a1a1a)]/25 mb-2 font-medium`}>Nama</label><input type="text" value={ni} onChange={e=>setNi(e.target.value)} className="w-full bg-white border border-[#1a1a1a]/8 rounded-xl px-5 py-3.5 text-sm text-[var(--theme-text,#1a1a1a)] focus:outline-none focus:border-[#1a1a1a]/25 transition-colors" placeholder="Nama Anda..."/></div>
                                    <div><label className={`${dmSans.className} block text-[9px] tracking-[.2em] uppercase text-[var(--theme-text,#1a1a1a)]/25 mb-2 font-medium`}>Ucapan</label><textarea value={mi} onChange={e=>setMi(e.target.value)} className="w-full bg-white border border-[#1a1a1a]/8 rounded-xl px-5 py-3.5 text-sm text-[var(--theme-text,#1a1a1a)] h-28 resize-none focus:outline-none focus:border-[#1a1a1a]/25 transition-colors" placeholder="Tulis ucapan..."/></div>
                                    <button type="submit" disabled={sub} className={`${dmSans.className} w-full bg-[var(--theme-bg,#1a1a1a)] text-[var(--theme-button-text,#ffffff)] py-4 rounded-xl text-[10px] tracking-[.3em] uppercase font-medium hover:bg-[var(--theme-accent,#333)] transition-colors disabled:opacity-50`}>{sub?'Mengirim...':'Kirim Ucapan'}</button>
                                </form>
                                {ws.length>0&&<div className="mt-8 space-y-3 max-h-[300px] overflow-y-auto sh">{ws.map((m,i)=>(<div key={i} className="bg-white rounded-2xl p-4 border border-[#1a1a1a]/5"><p className="text-sm text-[var(--theme-text,#1a1a1a)]/55 font-light">{m.message}</p><div className="flex items-center gap-2 mt-2"><div className="w-6 h-6 rounded-full bg-[var(--theme-bg,#1a1a1a)] flex items-center justify-center"><span className="text-white text-[10px] font-bold">{m.name?.charAt(0)?.toUpperCase()}</span></div><p className="text-xs text-[var(--theme-text,#1a1a1a)]/30">{m.name}</p></div></div>))}</div>}
                            </div>
                        </section>

                        {invitation?.gift_accounts&&invitation.gift_accounts.length>0&&<section className="px-8 pb-20 m3-rv"><div className="text-center mb-12"><h2 className={`${dmSerif.className} text-3xl text-[var(--theme-text,#1a1a1a)]`}>Wedding Gift</h2></div><div className="space-y-4">{invitation.gift_accounts.map((acc, i) => (
                                <GiftAtmCard key={acc.id || i} acc={acc} delayData={`${(i % 3) + 1}`} variant="MonoChromeIII" />
                            ))}</div></section>}

                        {/* ── FOOTER ── */}
                <footer className="bg-[var(--theme-bg,#1a1a1a)] text-[var(--theme-text,#1a1a1a)] pt-64 pb-24 px-8 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 pt-10">
                        ...
                        <h2 className={`font-serif text-5xl mb-4 text-[var(--theme-text,#1a1a1a)] drop-shadow-sm`}>
                        ...
                        <div className="border-t border-[#1a1a1a]/10 pt-8 mt-12">
                            <p className="text-[9px] text-[var(--theme-text,#1a1a1a)]/40 tracking-[0.2em] uppercase mb-2">Digital Invitation by</p>
                            ...
                            <p className="text-[8px] text-[var(--theme-text,#1a1a1a)]/30 mt-2 tracking-wider">© {new Date().getFullYear()} Digivitation. All rights reserved.</p>
                        </div>
                    </div>
                </footer>
                    </div>
