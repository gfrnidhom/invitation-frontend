...
export default function FrostedElegance({ payload }) {
            ...
            <div className={`transition-opacity duration-1000 ${isOpen?'opacity-100':'opacity-0 pointer-events-none'}`}>
                <div className="flex flex-col lg:flex-row min-h-screen">
                    <div className="sl-fe w-full lg:w-[55%] bg-[#1a1f2e] relative flex flex-col justify-end p-8 md:p-12 lg:p-16">
                        ...
                        {invitation?.music_url&&<button onClick={ta} className="absolute bottom-8 right-8 lg:bottom-1/2 lg:-right-6 lg:translate-y-1/2 z-50 w-12 h-12 rounded-full bg-[#1a1f2e] border border-white/15 flex items-center justify-center hover:border-white/30 transition-all shadow-2xl">{isPlaying?<svg className="w-5 h-5 ms-fe ib" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2z"/></svg>:<svg className="w-5 h-5 ib" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}</button>}
                    </div>
                    ...
                    <div ref={rpRef} className="w-full lg:w-[45%] lg:h-screen lg:overflow-y-auto sh bg-[#1a1f2e]">
                        ...
                        {phs.length>0&&<section className="pb-20"><div className="text-center mb-12 px-8 md:px-12 fe-rv"><h2 className={`${ci.className} text-2xl md:text-3xl font-light tracking-[.15em] uppercase text-white`}>Our Best</h2><p className={`${ci.className} text-4xl ib font-light -mt-1`}>Moments</p></div><div className="flex overflow-x-auto gap-4 px-8 md:px-12 pb-6 sh snap-x">{phs.slice(0,8).map((ph,i)=>(<div key={i} className="flex-none w-56 md:w-72 aspect-[3/4] snap-center rounded-2xl overflow-hidden group fc fe-rv"><img src={gp(ph)} alt={`Moment ${i+1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"/></div>))}</div></section>}
