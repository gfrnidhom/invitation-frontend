...
export default function MonoChromeIV({ payload, audioController }) {
    ...
    return (
        <div className={`min-h-screen bg-[var(--theme-bg,#18181b)] text-[var(--theme-text,#e4e4e7)] ${workSans.className} overflow-hidden`}>
            ...
            <div className={`co4 ${isOpen?'open':''}`}>
                <div className="relative w-full h-full flex flex-col items-center justify-center bg-[#09090b]">
                    ...
                    <div className="relative z-10 text-center px-6 flex flex-col items-center">
                        <div className="flex items-center gap-4 mb-8"><div className="w-16 h-px bg-[var(--theme-accent,#a1a1aa)]/30"/><div className="w-2 h-2 border border-[var(--theme-accent,#a1a1aa)]/30 rotate-45"/><div className="w-16 h-px bg-[var(--theme-accent,#a1a1aa)]/30"/></div>
                        <p className={`${spaceGrotesk.className} text-[10px] tracking-[.6em] uppercase stl mb-6 font-medium`}>The Wedding Of</p>
                        <h1 className={`${italiana.className} text-6xl md:text-8xl lg:text-[9rem] text-[var(--theme-text,#e4e4e7)] mb-2 leading-[.9]`}>{invitation?.groom_name?.split(' ')[0]}</h1>
                        <div className="flex items-center gap-4 my-3"><div className="w-12 h-px bg-[var(--theme-accent,#a1a1aa)]/20"/><span className={`${italiana.className} text-3xl stl`}>&</span><div className="w-12 h-px bg-[var(--theme-accent,#a1a1aa)]/20"/></div>
            ...
            <div className={`transition-opacity duration-1000 ${isOpen?'opacity-100':'opacity-0 pointer-events-none'}`}>
                <div className="flex flex-col lg:flex-row min-h-screen">
                    {/* LEFT */}
                    <div className="sl4 w-full lg:w-[70%] bg-[var(--theme-bg,#18181b)] relative flex flex-col justify-end p-8 md:p-12 lg:p-16">
                        ...
                        {invitation?.music_url&&<MusicPlayer audioController={audioController} btnBg="bg-[var(--theme-bg,#18181b)]" btnColor="stl" btnBorder="border-[var(--theme-accent,#a1a1aa)]/12 shadow-2xl" />}
                    </div>

                    {/* RIGHT */}
                    <div ref={rpRef} className="w-full lg:w-[30%] lg:h-screen lg:overflow-y-auto sh bg-[var(--theme-bg,#18181b)]">
                        <section className="py-20 px-8 text-center m4-rv">
                            ...
                            <h2 className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)] mb-2`}>{invitation?.groom_name?.split(' ')[0]} & {invitation?.bride_name?.split(' ')[0]}</h2>
                            <p className={`${spaceGrotesk.className} text-[9px] text-[var(--theme-text,#e4e4e7)]/15 tracking-[.3em] uppercase mb-10 font-light`}>{ed.toLocaleDateString('id-ID',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</p>
                            <div className="grid grid-cols-4 gap-2 max-w-sm mx-auto">{[{v:cd.d,l:'DAYS'},{v:cd.h,l:'HRS'},{v:cd.m,l:'MIN'},{v:cd.s,l:'SEC'}].map((it,i)=>(<div key={i} className="m4c py-4 px-2"><p className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)]`}>{it.v}</p><p className={`${spaceGrotesk.className} text-[7px] uppercase tracking-[.3em] stl mt-1 font-medium`}>{it.l}</p></div>))}</div>
                        </section>
                        ...
                        <section className="px-8 pb-20">
                            <div className="text-center mb-12 m4-rv"><h2 className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)]`}>Bride & Groom</h2><p className={`${spaceGrotesk.className} text-[9px] text-[var(--theme-text,#e4e4e7)]/12 tracking-[.3em] uppercase mt-2 font-medium`}>Assalamualaikum Wr. Wb.</p><p className={`${spaceGrotesk.className} text-[9px] text-[var(--theme-text,#e4e4e7)]/20 mt-3 max-w-md mx-auto leading-relaxed font-light`}>Tanpa mengurangi rasa hormat, kami bermaksud mengundang Bapak/Ibu/Saudara/i untuk menghadiri acara pernikahan kami:</p></div>
                            ...
                            <div className="m4c p-6 mb-6 m4-rv" data-delay="1">
                                <div className="flex flex-col md:flex-row items-center gap-6">
                                    ...
                                    <div className="text-center flex-1 line-accent pl-5">
                                        ...
                                        <h3 className={`${spaceGrotesk.className} text-lg tracking-[.05em] text-[var(--theme-text,#e4e4e7)] mb-3 font-medium`}>{invitation?.bride_full_name||invitation?.bride_name}</h3>
                                        <p className="text-xs text-[var(--theme-text,#e4e4e7)]/25 font-light">Putri {invitation?.bride_child_order ? `${invitation.bride_child_order} ` : ""}dari {invitation?.bride_father||'Bapak'} & {invitation?.bride_mother||'Ibu'}</p>
                                        {invitation?.bride_instagram && (
                                            <a href={`https://instagram.com/${invitation.bride_instagram}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-[var(--theme-text,#e4e4e7)]/50 hover:text-white transition-colors font-medium">
                            ...
                            <div className="flex items-center justify-center gap-3 my-5 m4-rv"><div className="w-px h-8 bg-[var(--theme-accent,#a1a1aa)]/10"/><div className="w-3 h-3 border border-[var(--theme-accent,#a1a1aa)]/15 rotate-45"/><div className="w-px h-8 bg-[var(--theme-accent,#a1a1aa)]/10"/></div>
                            <div className="m4c p-6 m4-rv" data-delay="2">
                                <div className="flex flex-col md:flex-row-reverse items-center gap-6">
                                    ...
                                    <div className="text-center flex-1">
                                        ...
                                        <h3 className={`${spaceGrotesk.className} text-lg tracking-[.05em] text-[var(--theme-text,#e4e4e7)] mb-3 font-medium`}>{invitation?.groom_full_name||invitation?.groom_name}</h3>
                                        <p className="text-xs text-[var(--theme-text,#e4e4e7)]/25 font-light">Putra {invitation?.groom_child_order ? `${invitation.groom_child_order} ` : ""}dari {invitation?.groom_father||'Bapak'} & {invitation?.groom_mother||'Ibu'}</p>
                                        {invitation?.groom_instagram && (
                                            <a href={`https://instagram.com/${invitation.groom_instagram}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-[var(--theme-text,#e4e4e7)]/50 hover:text-white transition-colors font-medium">
                                                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                                                @{invitation.groom_instagram}
                                            </a>
                                        )}
                        ...
                        <section className="px-8 pb-20">
                            <div className="text-center mb-12 m4-rv"><h2 className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)]`}>Wedding Event</h2></div>
                            {invitation?.events&&invitation.events.length>0?[...invitation.events].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)).map((ev,idx)=>(
                                <div key={idx} className="m4cw p-8 mb-6 text-center m4-rv" data-delay={`${idx+1}`}>
                                    ...
                                    {getMapUrl(ev)&&<MapLocationButton item={ev} className={`${spaceGrotesk.className} inline-flex items-center gap-2 bg-[var(--theme-bg,#18181b)] text-[var(--theme-text,#e4e4e7)] px-6 py-3 text-[9px] tracking-[.2em] uppercase font-medium hover:bg-[#27272a] transition-all duration-500 mt-4`} buttonText="Lihat Lokasi"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"/></svg><span>Lihat Lokasi</span></MapLocationButton>}
                                </div>
                            )):<div className="m4cw p-10 text-center"><h3 className={`${italiana.className} text-xl text-[#18181b] mb-2`}>Acara Pernikahan</h3></div>}
                        </section>

                        {invitation?.love_stories&&invitation.love_stories.length>0&&<section className="px-8 pb-20"><div className="text-center mb-12 m4-rv"><h2 className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)]`}>Our Love Story</h2></div>{[...invitation.love_stories].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)).map((s,i)=>(<div key={s.id||i} className="m4cw p-8 text-center mb-6 m4-rv" data-delay={`${i+1}`}><h3 className={`${italiana.className} text-lg text-[#18181b] mb-4`}>{s.title}</h3><p className="text-sm text-[#18181b]/50 leading-relaxed font-light">{s.description}</p>{s.photo&&<div className="mt-6 overflow-hidden"><img src={gp(s.photo)} alt={s.title} className="w-full h-44 object-cover grayscale"/></div>}</div>))}</section>}
                        ...
                        <div className="px-8"><QrCheckin guest={guest} sectionBg="bg-transparent" titleFont={italiana.className} textColor="text-[var(--theme-text,#e4e4e7)]" borderStyle="border-[var(--theme-accent,#a1a1aa)]/10"/></div>

                        <section className="px-8 pb-20 m4-rv">
                            <div className="text-center mb-12"><h2 className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)]`}>Wedding Wishes</h2></div>
                            <div className="m4cw p-8">
                                <form onSubmit={sw} className="space-y-4">
                                    ...
                                    <button type="submit" disabled={sub} className={`${spaceGrotesk.className} w-full bg-[var(--theme-bg,#18181b)] text-[var(--theme-text,#e4e4e7)] py-4 text-[9px] tracking-[.3em] uppercase font-medium hover:bg-[#27272a] transition-colors disabled:opacity-50`}>{sub?'Mengirim...':'Kirim Ucapan'}</button>
                                </form>
                                {ws.length>0&&<div className="mt-8 space-y-3 max-h-[300px] overflow-y-auto sh">{ws.map((m,i)=>(<div key={i} className="bg-[#fafafa] p-4 border border-[#18181b]/5"><p className="text-sm text-[#18181b]/55 font-light">{m.message}</p><div className="flex items-center gap-2 mt-2"><div className="w-6 h-6 bg-[var(--theme-bg,#18181b)] flex items-center justify-center"><span className="text-white text-[9px] font-bold">{m.name?.charAt(0)?.toUpperCase()}</span></div><p className="text-xs text-[#18181b]/25">{m.name}</p></div></div>))}</div>}
                            </div>
                        </section>

                        {invitation?.gift_accounts&&invitation.gift_accounts.length>0&&<section className="px-8 pb-20 m4-rv"><div className="text-center mb-12"><h2 className={`${italiana.className} text-3xl text-[var(--theme-text,#e4e4e7)]`}>Wedding Gift</h2></div><div className="space-y-4">{invitation.gift_accounts.map((acc, i) => (
                                <GiftAtmCard key={acc.id || i} acc={acc} delayData={`${(i % 3) + 1}`} variant="MonoChromeIV" />
                            ))}</div></section>}

                        {/* ── FOOTER ── */}
                <footer className="bg-[#1a1a1a] text-[var(--theme-text,#e4e4e7)] pt-64 pb-24 px-8 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 pt-10">
                        <p className={`${spaceGrotesk.className} text-[10px] text-white/50 tracking-[0.3em] uppercase font-bold mb-4`}>
                            Thank you for being part of our special day
                        </p>
                        <h2 className={`font-serif text-5xl mb-4 text-[var(--theme-text,#e4e4e7)] drop-shadow-sm`}>
                            {invitation?.groom_name?.split(' ')[0]} <span className="text-white/50 font-light mx-2">&</span> {invitation?.bride_name?.split(' ')[0]}
                        </h2>
                        
                        {/* Branding */}
                        <div className="border-t border-[#e4e4e7]/10 pt-8 mt-12">
                            <p className="text-[9px] text-[var(--theme-text,#e4e4e7)]/40 tracking-[0.2em] uppercase mb-2">Digital Invitation by</p>
                            ...
                            <p className="text-[8px] text-[var(--theme-text,#e4e4e7)]/30 mt-2 tracking-wider">© {new Date().getFullYear()} Digivitation. All rights reserved.</p>
                        </div>
                    </div>
                </footer>
                    </div>
