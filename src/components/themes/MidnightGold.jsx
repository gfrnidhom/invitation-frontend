            ...
            <div className={`transition-opacity duration-1000 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    ...
                    <div ref={rightPanelRef} className="w-full lg:w-[30%] lg:h-screen lg:overflow-y-auto scrollbar-hide bg-[#0c1220]">
                                ...
                                <div className="flex flex-col md:flex-row items-center gap-6">
                                    ...
                                    <div className="text-center flex-1">
                                        ...
                                        {invitation?.bride_instagram && (
                                            <a href={`https://instagram.com/${invitation.bride_instagram}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-white/60 hover:text-[#d4af37] transition-colors font-medium">
                                ...
                                <div className="flex flex-col md:flex-row-reverse items-center gap-6">
                                    ...
                                    <div className="text-center flex-1">
                                        ...
                                        {invitation?.groom_instagram && (
                                            <a href={`https://instagram.com/${invitation.groom_instagram}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-white/60 hover:text-[#d4af37] transition-colors font-medium">
                                                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                                                @{invitation.groom_instagram}
                                            </a>
                                        )}
                                    ...
                                    <div key={story.id || i} className="card-midnight rounded-3xl p-8 text-center mb-6 mg-reveal" data-delay={`${i+1}`}>
                                        ...
                                        {story.photo && (
                                            <div className="mt-6 rounded-xl overflow-hidden gold-border">
                                                <img src={((img) => { let p = Array.isArray(img) ? img[0] : img; p = typeof p === 'object' && p !== null ? p.photo || p.url : p; if (typeof p !== 'string') return null; return (p.startsWith('http') || p.startsWith('/')) ? p : `${process.env.NEXT_PUBLIC_STORAGE_URL || 'https://digitvitation.my.id/storage'}/${p}`; })(story.photo)} alt={story.title} className="w-full h-44 object-cover" />
                                            </div>
                                        )}
                                    </div>
                                ))}
                        ...
                        {invitation?.gift_accounts && invitation.gift_accounts.length > 0 && (
                            <section className="px-8 pb-20 mg-reveal">
                                ...
                                <GiftAtmCard key={acc.id || i} acc={acc} delayData={`${(i % 3) + 1}`} variant="MidnightGold" />
                            ))}
                                </div>
                            </section>
                        )}
                ...
                <footer className="bg-[#1a1a1a] text-white pt-64 pb-24 px-8 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10 pt-10">
                        <p className={`${lato.className} text-[10px] text-white/50 tracking-[0.3em] uppercase font-bold mb-4`}>
