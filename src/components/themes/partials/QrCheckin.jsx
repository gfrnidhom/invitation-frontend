...
export default function QrCheckin({ guest, sectionBg = 'bg-white', titleFont = 'font-bold', textColor = 'text-gray-900', borderStyle = 'border-gray-200' }) {
  if (!guest || !guest.qr_code) return null;

  const storageUrl = process.env.NEXT_PUBLIC_STORAGE_URL || 'http://127.0.0.1:8000/storage';
  
  // If it's a generated external dummy (e.g starting with http), use it directly, else prepend storage URL
    ...
    <section className={`py-20 px-6 ${sectionBg} text-center reveal`}>
      <div className={`max-w-sm mx-auto p-8 rounded-3xl border ${borderStyle} shadow-sm bg-white`}>
        ...
        <div className="bg-gray-50 p-4 rounded-2xl flex justify-center mb-4 border border-gray-100">
          <img src={qrUrl} alt="QR Code Check in" className="w-48 h-48 object-contain mix-blend-multiply" />
        </div>
        
        <p className={`text-xs uppercase tracking-widest font-semibold ${textColor}`}>{guest.name}</p>
      </div>
    </section>
  );
}
